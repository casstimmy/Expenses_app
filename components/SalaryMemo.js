...
const SalaryMemo = forwardRef(({ staffList = [], onDownloading }, ref) => {
      ...
      >
        {chunkedStaff.map((group, groupIndex) => {
          ...
          const processedGroup = group.map((staff) => {
            const netSalary = (staff.salary || 0) - (
          ...
          return (
            <div key={groupIndex} style={{ pageBreakAfter: "always" }}>
              ...
              <div style={{ paddingTop: "5.8rem", position: "relative", zIndex: 2 }}>
                <p style={{ fontWeight: "bold", fontSize: "14px" }}>{today}</p>
                ...
                <p style={{ fontWeight: "500", marginBottom: "1.3rem", marginTop: "2rem", fontSize: "14px" }}>
                  Dear Sir,
                </p>

                <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                  <p style={{ textDecoration: "underline", fontWeight: "bold", marginBottom: "0.5rem", fontSize: "14px" }}>
                    ATTENTION: WILLIAMS CHEKE
                  </p>
                  <p style={{ fontWeight: "500", fontSize: "14px" }}>
                    WITHDRAWAL FROM OUR ACCOUNT 1239069143 IBILE TRADING RESOURCES LIMITED
                  </p>
                </div>

                <p style={{ marginBottom: "1.5rem" }}>
                  Please debit our account <strong>1239069143</strong> with the sum of <strong>₦{totalChunk.toLocaleString()}</strong>
                  <span className="px-1">
                    ({toWords(totalChunk).replace(/\b\w/g, (c) => c.toUpperCase())}
                  </span>{" "}
                ...
                >
                  <thead>
                    <tr style={{ backgroundColor: "#f5f7fa", textAlign: "left" }}>
                      ...
                      <th style={{ ...th, fontSize: "14px" }}>Account Name</th>
                      <th style={{ ...th, fontSize: "14px" }}>Account Number</th>
                  ...
                  <tbody>
                    {processedGroup.map((staff, i) => (
                      <tr key={staff._id}>
                        ...
                        <td style={td}>{staff.bank?.accountNumber || "N/A"}</td>
                        <td style={td}>{staff.bank?.bankName || "N/A"}</td>
                        <td style={td}>₦{staff.netSalary.toLocaleString()}</td>
                      </tr>
                    ))}
                    <tr style={{ backgroundColor: "#f0f4f8", borderTop: "2px solid #ddd" }}>
                      <td colSpan={5} style={{ ...td, fontWeight: "bold" }}>Total</td>
                      <td style={{ ...td, fontWeight: "bold" }}>₦{totalChunk.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div className="mt-12">
                  ...
                  <p style={{ fontWeight: "bold", paddingTop: "3em", fontSize: "14px" }}>Paul Farrer</p>
                  <p style={{ fontWeight: "bold", fontSize: "14px" }}>Director</p>
                </div>
              </div>
