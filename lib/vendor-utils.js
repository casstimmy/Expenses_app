...
export function buildPettyCashOnboardingProducts(
  products = [],
  fallbackCategory = ""
) {
  if (!Array.isArray(products)) {
    return { products: [], hasIncompleteRows: false };
  }

  const defaultCategory = cleanString(fallbackCategory) || "Petty Cash";

  const normalizedRows = products.map((product = {}) => {
    const name = cleanString(product.name || product.productName || product.product);
    const price = parsePositiveNumber(product.price);

    return {
      name,
      category: cleanString(product.category) || defaultCategory,
      price,
    };
  });

  const hasIncompleteRows = normalizedRows.some(
    ({ name, price }) => (name || price !== null) && (!name || price === null || price <= 0)
  );

  return {
    hasIncompleteRows,
    products: normalizedRows
      .filter(({ name, price }) => name && price !== null && price > 0)
  ...
  return Promise.all(
    products.map(async (prod) => {
      ...
      if (product && product !== "custom") {
        ...
        if (existingProduct) {
          existingProduct.isPack = !!isPack;
          existingProduct.unitsPerPack = isPack ? Number(unitsPerPack) || 1 : 1;
          if (Number(price ?? costPrice) > 0) {
            existingProduct.costPrice = Number(price ?? costPrice) || 0;
          }
