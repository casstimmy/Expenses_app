...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "update-details") {
      ...
      const nextRequestDate = parseDate(requestDate, transaction.requestDate);
      ...
      if (String(transaction.vendor) !== String(vendor._id)) changedFields.push("vendor");
      if (transaction.purpose !== nextPurpose) changedFields.push("purpose");
      if ((transaction.description || "") !== nextDescription) changedFields.push("note");
      if (Number(transaction.quantity || 1) !== normalizedOrder.quantity) changedFields.push("quantity");
      if (Number(transaction.unitPrice || 0) !== normalizedOrder.unitPrice) changedFields.push("unit price");
      if (Number(transaction.amount || 0) !== normalizedOrder.amount) changedFields.push("total amount");
      if (transaction.location !== nextLocation) changedFields.push("location");
      if (new Date(transaction.requestDate).getTime() !== nextRequestDate?.getTime()) {
        changedFields.push("order date");
      }
      if ((transaction.neededBy?.getTime?.() || 0) !== (nextNeededBy?.getTime?.() || 0)) {
        changedFields.push("needed by");
      }
      ...
      transaction.approvalHistory.push(
        buildApprovalHistoryEntry({
          ...
          note: typeof note === "string" && note.trim() ? note : updateNote,
          staff: authStaff,
